...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const mw = mapImg.naturalWidth;  // image width
    ...
    const mrh = mapImg.height; // actual image height
    const ratio = mrw/mw;
    const centerMap=[2350,1150];
    const altezzaAreaDaMostrare = 1300;
    const altezzaMappaVista = Math.max(600,window.innerHeight);
    const scale = mh/altezzaAreaDaMostrare;



    mapSection.style.transform = `translate(0,0)`;
    console.log(ratio);
    console.log(altezzaMappaVista,mrw*scale,mrh);
    const oPoints = [
      {	
        x: 2800,
        y: 1522
    ...
    // const nPoints = [
    //   {	
    //     x: ((oPoints[0].x - (centerMap[0]-mapViewedZoneWidth/2))/mapViewedZoneWidth)*mapRealWidth,
    //     y: ((oPoints[0].y - (centerMap[1]-mapViewedZoneWidth/2))/mapViewedZoneWidth)*Math.min(600,window.innerHeight ),
    //   },
    //   {	
    //     x: ((oPoints[1].x - (centerMap[0]-mapViewedZoneWidth/2))/mapViewedZoneWidth)*mapRealWidth,
    //     y: ((oPoints[1].y - (centerMap[1]-mapViewedZoneWidth/2))/mapViewedZoneWidth)*Math.min(600,window.innerHeight ),

    //   },
    //   {	
    //     x: ((oPoints[2].x - (centerMap[0]-mapViewedZoneWidth/2))/mapViewedZoneWidth)*mapRealWidth,
    //     y: ((oPoints[2].y - (centerMap[1]-mapViewedZoneWidth/2))/mapViewedZoneWidth)*Math.min(600,window.innerHeight ),

    //   }
    // ];
    console.log("ciao"+ratio);

  mapImg.style.transformOrigin = `${centerMap[0]*ratio}px ${centerMap[1]*ratio}px`;
  mapImg.style.transform = `scale(${scale}) `;

  const titleContainerDesktop = document.querySelector(".title-container-desktop");
  locations.forEach((location, index) => {
    // Creazione del div esterno
    let titleBoxDiv = titleContainerDesktop.querySelector("#title-box-"+index);
    console.log(location.index, titleBoxDiv);
    // titleBoxDiv.style.transform = `translate(${nPoints[0].x}px ${nPoints[0].y}px) `;
        // titleBoxDiv.style.transform = `translate(${nPoints[index].x}px, ${nPoints[index].y}px) `;


    
});
  titleBox.innerHTML = `
  <div><h3 style="text-align: center; margin: 0; padding: 0;">${locations[2].place}</h3></div>
`;
